...
使用一致的命名模式，格式为 `wubc-<动名词形式>`。

### 格式要求
- **必须**以 `wubc-` 为前缀
- 前缀后使用 **动名词形式** (verb + -ing)，如 `wubc-updating-tasks`
- 仓库名自动对应：`github.com/wubincai/wubc-<动名词>`

**好的命名示例:**
- `wubc-updating-tasks`
- `wubc-creating-reports`
- `wubc-processing-files`
- `wubc-analyzing-data`
- `wubc-generating-documents`

**可接受的替代（前缀后）:**
- 名词短语: `wubc-task-updating`, `wubc-file-processing`
- 动作导向: `wubc-task-update`, `wubc-file-process`

**避免:**
- 缺少 `wubc-` 前缀: `processing-pdfs`, `updating-tasks`
